...
dotnet add package Mapgen.Analyzer.Abstractions
```
...
```xml
<ItemGroup>
    <PackageReference Include="Mapgen.Analyzer.Abstractions" Version="1.0.0" />
...
**Note:** The `Mapgen.Analyzer` package must be configured as an analyzer with `OutputItemType="Analyzer"` and `ReferenceOutputAssembly="false"`.
